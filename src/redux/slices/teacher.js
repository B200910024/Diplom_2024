...
const slice = createSlice({
  ...
  reducers: {
    ...
    // GETTING RATING LIST
    getFamousTeacherList(state, action) {
      state.famousTeacherList = action.payload;
    },
...
export function fetchTeacherList(body = {}) {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await axiosGeneral.get('/teachers', body);
      dispatch(slice.actions.getTeacherList(response.data));
    } catch (error) {
...
export function searchTeacher(body) {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const teacherName = body?.teacher?.firstName;
      const response = await axiosGeneral.get(`/teacher-by?teacherName=${teacherName}`);
...
export function fetchFamousTeacherList() {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await axiosGeneral.get('/teacher-with-rating?sort=ratingCount');
      dispatch(slice.actions.getFamousTeacherList(response.data));
    } catch (error) {
      dispatch(slice.actions.hasError(error));
    }
  };
